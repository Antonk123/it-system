# Versioned Database Migrations Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Replace the 22 procedural `ensure*` functions in `connection.ts` with a versioned migration system so new schema changes run exactly once and are tracked in the database.

**Architecture:** A `schema_migrations` table records which migrations have been applied (by string ID). On startup, the runner compares the ordered migrations list against applied IDs and runs only pending ones. Migrations keep their idempotency guards (`tableExists`/`columnExists`) so existing databases transition safely — on first run all historical migrations execute (their guards make them no-ops on existing data) and are marked applied. Subsequent startups skip them entirely.

**Tech Stack:** better-sqlite3, TypeScript, Node.js ESM
...
| File | Action | Purpose |
|------|--------|---------|
| `server/src/db/migrations.ts` | **Create** | Ordered array of all 23 migrations (converted from `ensure*` functions) |
| `server/src/db/connection.ts` | **Modify** | Add migration runner + `schema_migrations` table, replace `initializeDatabase`, remove all `ensure*` functions |

---

## Task 1: Create `migrations.ts` with all historical migrations

**Files:**
- Create: `server/src/db/migrations.ts`

Each migration has:
- `id` — zero-padded string (`'001'` … `'023'`), primary key in `schema_migrations`
- `name` — human-readable label
- `up(db, helpers)` — runs the schema change; keeps idempotency guards

`tableExists`/`columnExists` are passed as `helpers` to avoid a circular import (since `connection.ts` imports `migrations.ts`).

- [ ] **Step 1: Create `server/src/db/migrations.ts`**
...
export const migrations: Migration[] = [
  ...
  },
  {
    ...
    up: (db, { tableExists }) => {
      ...
      db.prepare(`CREATE TRIGGER IF NOT EXISTS update_comment_updated_at
  ...
  },
  {
    ...
    up: (db, { tableExists }) => {
      ...
      ['idx_ticket_reminders_ticket ON ticket_reminders(ticket_id)',
       'idx_ticket_reminders_user ON ticket_reminders(user_id)',
       'idx_ticket_reminders_time ON ticket_reminders(reminder_time)',
       'idx_ticket_reminders_sent ON ticket_reminders(sent)',
      ].forEach(idx => db.prepare(`CREATE INDEX IF NOT EXISTS ${idx}`).run());
    },
  },
  {
    id: '012',
  ...
  },
  {
    ...
    up: (db, { tableExists, columnExists }) => {
      ...
      if (!tableExists('kb_articles_fts'))
        db.prepare(`CREATE VIRTUAL TABLE IF NOT EXISTS kb_articles_fts USING fts5(title, content_plain, content='', tokenize='unicode61')`).run();
...
```bash
wc -l server/src/db/migrations.ts
# Expected: ~250+ lines
```
...
git commit -m "feat(db): add versioned migrations array (23 historical migrations)"
```

---

## Task 2: Add migration runner to `connection.ts`

**Files:**
- Modify: `server/src/db/connection.ts`

- [ ] **Step 1: Add import at the top of `connection.ts`**
...
- [ ] **Step 2: Add `createSchemaMigrationsTable` and `runMigrations` functions**

Add these two functions directly above `initializeDatabase`:
...
- [ ] **Step 3: Replace `initializeDatabase` body**
...
Note: `schema.sql` uses multi-statement SQL so it needs `db.exec` not `db.prepare`. Keep using `db.exec(schema)` here — only the migration runner uses `db.prepare().run()`:

```typescript
export function initializeDatabase() {
  const schemaPath = join(__dirname, 'schema.sql');
  const schema = readFileSync(schemaPath, 'utf-8');
  // schema.sql contains multi-statement DDL — exec handles multiple statements
  // (this is the only place exec is used; all migrations use db.prepare().run())
  db.exec(schema);
  runMigrations();
  console.log('Database initialized successfully');
}
...
- [ ] **Step 4: Delete all 22 `ensure*` functions from `connection.ts`**
...
Also remove the `stripHtml` import (line 6) — it's now only used in `migrations.ts`.
...
If errors appear, they will be missing imports or wrong types — fix them before continuing.

- [ ] **Step 6: Commit**

```bash
git add server/src/db/connection.ts
git commit -m "refactor(db): replace ensure* functions with versioned migration runner"
```

---

## Task 3: Smoke test on existing database
...
- [ ] **Step 2: Run server against the dev database**

```bash
NODE_ENV=development JWT_SECRET=any-test-value DB_PATH=./data/database.sqlite node dist/index.js 2>&1 | head -40
```

First run expected output (migrations running on existing DB — guards fire, no-ops):
```
Running migration 001: ensure_ticket_attachments_table
Migration 001 applied
...
Running migration 023: ensure_tickets_closed_at_index
Migration 023 applied
Database initialized successfully
```

Subsequent run (all already applied):
```
Database initialized successfully
```
...
- [ ] **Step 3: Verify `schema_migrations` table is populated**

```bash
sqlite3 ./data/database.sqlite "SELECT id, name FROM schema_migrations ORDER BY id;"
```

Expected: 23 rows (001 through 023).

- [ ] **Step 4: Commit**

```bash
git add -A
git commit -m "test(db): verify versioned migration runner on existing database"
```
...
To add a new schema change, append to the `migrations` array in `server/src/db/migrations.ts`:

```typescript
{
  id: '024',
  name: 'add_ticket_priority_index',
  up: (db) => {
    db.prepare('CREATE INDEX IF NOT EXISTS idx_tickets_priority ON tickets(priority)').run();
  },
},
```

**Rules:**
- IDs must be unique and higher than any existing ID
- Never edit existing migration entries — add new ones only
- Keep idempotency guards (`tableExists`, `columnExists`) if there's any chance the object already exists
