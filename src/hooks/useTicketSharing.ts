...
export const useTicketSharing = () => {
  ...
  const createShareLink = useCallback(async (ticketId: string, expiresInDays?: number): Promise<string | null> => {
    ...
    setIsError(false);
    try {
      const { share_token, expires_at } = await api.createShareToken(ticketId, expiresInDays);
  ...
  const deleteShareLink = useCallback(async (ticketId: string): Promise<boolean> => {
    ...
    setIsPending(true);
    setIsError(false);
    try { await api.deleteShareToken(ticketId); setShareUrl(null); setExpiresAt(null); toast.success('Delningslänk borttagen'); return true; }
