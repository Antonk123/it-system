      ...
      <div className="max-w-3xl mx-auto p-4 sm:p-6 lg:p-8 space-y-6">
        ...
        <Card>
          ...
          <CardContent className="space-y-6">
            <div><h2 className="font-medium text-foreground mb-2">Beskrivning</h2><div className="bg-muted/30 p-4 rounded-lg"><HtmlRenderer content={migrateContent(ticket.description)} /></div></div>
            {checklistItems.length > 0 && <div className="pt-4 border-t"><h2 className="font-medium text-foreground mb-3">Checklista</h2><div className="border rounded-lg p-4 space-y-2">{checklistItems.map((item) => <div key={item.id} className="flex items-center gap-3">{item.completed ? <CheckCircle2 className="w-5 h-5 text-primary shrink-0" /> : <Circle className="w-5 h-5 text-muted-foreground shrink-0" />}<span className={item.completed ? 'line-through text-muted-foreground' : ''}>{item.label}</span></div>)}</div></div>}
            {attachments.length > 0 && <div className="pt-4 border-t"><div className="flex items-center gap-2 mb-3"><Paperclip className="w-4 h-4 text-muted-foreground" /><h2 className="font-medium text-foreground">Bilagor ({attachments.length})</h2></div><div className="grid grid-cols-1 sm:grid-cols-2 gap-3">{attachments.map((attachment) => { const isImage = attachment.file_type?.startsWith('image/'); if (!attachment.url) return null; return <a key={attachment.id} href={attachment.url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 p-3 bg-muted/50 rounded-lg hover:bg-muted transition-colors group">{isImage ? <img src={attachment.url} alt={attachment.file_name} className="w-12 h-12 object-cover rounded" /> : <div className="w-12 h-12 flex items-center justify-center bg-muted rounded"><FileText className="w-6 h-6 text-muted-foreground" /></div>}<div className="flex-1 min-w-0"><p className="text-sm font-medium truncate group-hover:underline">{attachment.file_name}</p><p className="text-xs text-muted-foreground">{formatFileSize(attachment.file_size)}</p></div><Download className="w-4 h-4 text-muted-foreground opacity-0 group-hover:opacity-100 transition-opacity" /></a>; })}</div></div>}
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-4 border-t">{requester && <div className="flex items-center gap-3"><UserIcon className="w-5 h-5 text-muted-foreground" /><div><p className="text-sm text-muted-foreground">Beställare</p><p className="font-medium">{requester.name}</p>{requester.company && <p className="text-sm text-muted-foreground">{requester.company}</p>}</div></div>}<div className="flex items-center gap-3"><Calendar className="w-5 h-5 text-muted-foreground" /><div><p className="text-sm text-muted-foreground">Skapad</p><p className="font-medium">{format(parseServerDate(ticket.created_at), 'PPP', { locale: sv })}</p></div></div><div className="flex items-center gap-3"><Clock className="w-5 h-5 text-muted-foreground" /><div><p className="text-sm text-muted-foreground">Senast uppdaterad</p><p className="font-medium">{format(parseServerDate(ticket.updated_at), 'PPP', { locale: sv })}</p></div></div></div>
            {ticket.solution && <div className="pt-4 border-t"><div className="flex items-center gap-2 mb-2"><Lightbulb className="w-4 h-4 text-primary" /><h2 className="font-medium text-foreground">Lösning</h2></div><div className="bg-primary/5 border border-primary/20 p-4 rounded-lg"><HtmlRenderer content={migrateContent(ticket.solution)} /></div></div>}
